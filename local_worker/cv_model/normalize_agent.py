...
# ── config loading ──
#
# The web repo resolves ats_config.yaml relative to its own repo root
# (os.path.dirname(os.path.dirname(__file__))), which assumes this module
# lives two directories under the repo root (agents/normalize_agent.py ->
# repo root). That assumption breaks once ported into local_worker/, which
# ships as its own self-contained folder/exe, not alongside the web repo.
#
# This mirrors qml_gui.py::resource_path()'s PyInstaller-frozen-vs-dev-run
# handling (sys._MEIPASS) instead of importing resource_path() itself,
# because that would pull qml_gui.py's PySide6 (GUI) dependency into the
# parsing/scoring code path — which must also run under plain pytest and
# the CLI (tests/test_local_worker_cli.py), neither of which requires a Qt
# runtime.
def _resolve_config_path() -> str:
    if hasattr(sys, "_MEIPASS"):
        base = Path(sys._MEIPASS)
    else:
        # local_worker/cv_model/normalize_agent.py -> local_worker/
        base = Path(__file__).resolve().parent.parent
    return str(base / "ats_config.yaml")
...
from .section_scorer import is_language_entry as _is_structural_lang
from .section_scorer import CEFR_RE as _LANG_LEVEL_RE
...
def normalize(structured: Dict) -> Dict:
    ...
    """
    from .section_classifier import canonicalize_section_key
    ...
    from .section_resolver import resolve_parsed_entries

    resolve_parsed_entries(normalized)

    # 7e. Sanitization rules (normalizer only sanitizes — no section moving)
    from .cv_normalizer import apply_normalization_rules

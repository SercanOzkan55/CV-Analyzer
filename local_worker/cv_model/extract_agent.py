...
from .section_classifier import detect_sections, canonicalize_section_key, get_parser
from .section_resolver import resolve_raw_sections
...
def _get_autofix_helpers():
    """Lazy import to avoid circular dependency with extraction_helpers."""
...
def _merge_wrapped_lines(text: str) -> str:
    ...
    for line in lines:
        ...
        if len(line.split()) <= 3 and line[0].isupper():
            from .section_classifier import _sniff_header
...
def extract_structured(cv_text: str) -> Dict:
    ...
    from .extraction_helpers import _parse_sections as _parse_sections_deterministic
    ...
    if not name:
        from .extraction_helpers import guess_name_from_lines

"""Skills-coverage (taxonomy) criterion, ported from services/skill_service.py.

Ports ``extract_skills`` and ``skill_coverage_score`` verbatim, including
the guarded Redis cache block. That block is a safe no-op whenever
``redis`` isn't importable (it always fails closed into pure in-memory
extraction) — kept as-is per the port instructions, but ``redis`` is
deliberately NOT added to local_worker/requirements.txt so this stays a
zero-network-dependency, offline-only path in the shipped worker.
"""
...
# extraction logic still works normally without caching.
_skills_redis = None
...
SKILL_CATEGORIES = {
    "languages": [
        ...
        "perl", "lua", "dart", "elixir", "haskell", "objective-c", "groovy", "clojure",
        "fortran", "cobol", "visual basic", "vb\\.net", "f#", "julia", "assembly",
    ],
    "frontend": [
        "react", "angular", "vue", "vue\\.js", "svelte", "next\\.js", "nextjs", "nuxt",
        "nuxt\\.js", "gatsby", "remix", "astro", "html", "css", "sass", "scss", "less",
        "tailwind", "tailwindcss", "bootstrap", "material ui", "mui", "chakra ui",
        "ant design", "jquery", "webpack", "vite", "babel", "rollup", "esbuild",
        "storybook", "redux", "zustand", "mobx", "pinia", "vuex", "three\\.js", "d3\\.js",
        "d3", "chart\\.js",
    ],
    "backend": [
        "node\\.js", "nodejs", "express", "express\\.js", "nestjs", "nest\\.js", "fastapi",
        "django", "flask", "spring", "spring boot", "springboot", "asp\\.net", "\\.net",
        "dotnet", "laravel", "symfony", "rails", "ruby on rails", "sinatra", "gin",
        "fiber", "echo", "actix", "rocket", "axum", "phoenix", "koa", "hapi", "graphql",
        "rest", "restful", "grpc", "websocket", "microservices", "serverless", "oauth", "jwt",
    ],
    "databases": [
        "sql", "postgresql", "postgres", "mysql", "mariadb", "sqlite", "oracle",
        "sql server", "mssql", "mongodb", "dynamodb", "cassandra", "couchdb", "neo4j",
        "redis", "memcached", "elasticsearch", "opensearch", "solr", "firebase", "supabase",
        "cockroachdb", "timescaledb", "influxdb", "clickhouse", "snowflake", "bigquery",
        "prisma", "sequelize", "sqlalchemy", "hibernate", "typeorm", "mongoose", "knex",
    ],
    "devops_cloud": [
        "docker", "kubernetes", "k8s", "podman", "aws", "amazon web services", "azure",
        "gcp", "google cloud", "terraform", "pulumi", "cloudformation", "ansible", "chef",
        "puppet", "jenkins", "github actions", "gitlab ci", "circle ci", "circleci",
        "travis ci", "argo cd", "argocd", "tekton", "nginx", "apache", "caddy", "haproxy",
        "linux", "ubuntu", "centos", "debian", "rhel", "bash", "shell", "powershell",
        "prometheus", "grafana", "datadog", "new relic", "splunk", "elk", "kibana",
        "logstash", "fluentd", "helm", "istio", "envoy", "consul", "vault",
    ],
    "data_ml": [
        ...
        "hugging face", "huggingface", "transformers", "bert", "gpt", "llm", "langchain",
        "openai", "spark", "pyspark", "hadoop", "hive", "airflow", "dbt", "kafka", "flink",
        "beam", "tableau", "power bi", "looker", "metabase", "etl", "data warehouse",
        "data lake", "data pipeline", "feature engineering", "model deployment", "mlops",
        "mlflow", "kubeflow", "sagemaker", "vertex ai", "onnx", "xgboost", "lightgbm",
    ...
    "testing": [
        ...
        "rspec", "junit", "testng", "postman", "insomnia", "k6", "locust", "jmeter", "tdd",
        "bdd", "cucumber", "robot framework", "sonarqube", "codecov",
    ],
    "tools_practices": [
        "git", "github", "gitlab", "bitbucket", "svn", "jira", "confluence", "trello",
        "asana", "notion", "linear", "figma", "sketch", "adobe xd", "invision", "agile",
        "scrum", "kanban", "ci/cd", "cicd", "api design", "system design", "design patterns",
        "solid", "clean architecture", "domain driven design", "ddd", "oauth2", "saml",
        "sso", "rbac",
    ],
    "security": [
        "owasp", "penetration testing", "vulnerability assessment", "sast", "dast", "siem",
        "ids", "ips", "encryption", "tls", "ssl", "pki", "nmap", "burp suite", "wireshark",
        "metasploit", "iso 27001", "soc 2", "gdpr", "hipaa", "pci dss", "zero trust",
        "firewall", "waf",
    ],
}
...
_ABBREVIATION_MAP: dict[str, str] = {
    ...
    "k8s": "kubernetes", "tf": "terraform", "gha": "github actions", "pg": "postgresql",
    "mongo": "mongodb", "es": "elasticsearch", "rn": "react native", "rds": "aws",
    "ec2": "aws", "s3": "aws", "ml": "machine learning", "dl": "deep learning",
    "cv": "computer vision", "ai": "machine learning", "ci": "ci/cd", "cd": "ci/cd",
    "nosql": "mongodb", "gke": "kubernetes", "eks": "kubernetes", "aks": "kubernetes",
    "cdk": "cloudformation", "sqs": "aws", "sns": "aws", "ecs": "aws", "drf": "django",
    "orm": "sqlalchemy",
}
...
def extract_skills(text: str) -> dict:
    ...
    try:
        STOPWORDS = {
            "and", "or", "with", "the", "a", "an", "for", "in", "on", "of", "to", "by",
            "experience", "years", "year", "skills", "skill", "tools", "using", "knowledge",
            "proficient", "familiar", "including", "etc", "and/or", "strong", "ability",
            "able", "responsible", "worked",
        }
...
def skill_coverage_score(cv_text: str, job_text: str):
    """Calculate skill coverage score. Returns (score, missing_skills_list)."""
...
def skills_coverage_score(cv_text: str, job_text: str) -> float:
    """Local Worker criterion entry point: 0-100 coverage score only."""
    score, _missing = skill_coverage_score(cv_text, job_text)
    return score

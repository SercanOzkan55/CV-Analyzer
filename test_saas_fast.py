...
# Test users
TEST_USER_1 = {"sub": "user-123-aaa", "email": "user1@example.com", "aud": "authenticated"}
TEST_USER_2 = {"sub": "user-456-bbb", "email": "user2@example.com", "aud": "authenticated"}
...
def test_8_quota_enforcement():
    ...
    try:
        ...
        for i in range(5):
            resp = requests.post(f"{API_BASE_URL}/api/v1/analyze", json=body, headers=headers, timeout=10)
            if resp.status_code != 200:
                print(f"✗ request {i+1} returned {resp.status_code}")
                success = False
                break
        # sixth request should be rejected
        resp6 = requests.post(f"{API_BASE_URL}/api/v1/analyze", json=body, headers=headers, timeout=10)
...
def run_all_tests():
    ...
    if not check_server():
        ...
        print(f"   Start: python -m uvicorn main:app --reload")
        return False

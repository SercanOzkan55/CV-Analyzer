...
def test_extract_linkedin_zip_error():
    ...
    with pytest.raises(ValueError, match="File content does not match ZIP format"):
...
def test_process_batch_worker(mock_calc, mock_extract):
    ...
    batch = [
        ...
        {"filename": "short.txt", "content": b"too short"},
    ]
...
def test_process_cv_batch_parallel(mock_persist, mock_ppe):
    async def _run_test():
        ...
        files = [
            {
                "filename": "test.txt",
                "content": b"Very long dummy text to pass the limit. Very long dummy text to pass the limit.",
            }
        ]

        with patch("utils.cv_processor._process_batch_worker") as mock_worker:
            mock_worker.return_value = [{"filename": "test.txt", "final_score": 80.0, "status": "success"}]
            ...
            res2 = await process_cv_batch_parallel(
                files, "Job Desc", 1, workers=1, persist=True, org_id=10, recruiter_id=20
            )
            assert len(res2) == 1
...
def test_cv_processing_cache():
    async def _run_test():
        ...
        await cache.cache_score(
            "hash3", 1, result
        )  # This shouldn't exceed or might not be saved depending on implementation
...
def test_process_cv_batch_ultra_fast(mock_get_cache, mock_parallel):
    async def _run_test():
        ...
        mock_cache.get_cached_score.side_effect = lambda h, j: (
            {"final_score": 95.0, "status": "success"} if h == hash_val else None
        )

        mock_parallel.return_value = [{"filename": "f2.txt", "final_score": 80.0, "status": "success"}]

        files = [{"filename": "f1.txt", "content": b"content1"}, {"filename": "f2.txt", "content": b"content2"}]
...
def test_process_cv_batch_uploadfile(mock_extract):
    async def _run_test():
        ...
        files = [
            DummyUploadFile(
                "test.txt",
                b"Very long dummy text to satisfy the 50 char limit requirement. Very long dummy text to satisfy the 50 char limit requirement.",
            ),
            DummyUploadFile("short.txt", b"too short"),
            DummyUploadFile(
                "fail.txt", b"throw exception in pipeline. throw exception in pipeline. throw exception in pipeline."
            ),
        ]
...
def test_process_linkedin_export(mock_process, mock_extract):
    async def _run_test():
        mock_extract.return_value = [{"filename": "test.txt", "content": b"content"}]
        mock_process.return_value = [{"filename": "test.txt", "status": "success", "final_score": 85.0}]
...
def test_extract_linkedin_zip_streaming():
    async def _run_test():
        ...
        mock_file = AsyncMock()
        mock_file.read.side_effect = [zip_buffer.getvalue(), b""]

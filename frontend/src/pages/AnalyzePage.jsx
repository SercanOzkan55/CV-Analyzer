...
import { isPdfUpload, isSupportedCvUpload } from '../utils/fileTypes'
...
export default function AnalyzePage() {
  ...
  async function handleAnalyze(e) {
    ...
    if (!isSupportedCvUpload(file)) return setError(t('analyze.invalid_file'))
    if (file.size > 5 * 1024 * 1024) return setError(t('analyze.file_too_large'))
  ...
  async function handleExportAutoFix(format) {
    ...
    try {
      ...
      const baseName = (file?.name || 'optimized_cv').replace(/\.(pdf|docx|txt)$/i, '')
      a.href = url

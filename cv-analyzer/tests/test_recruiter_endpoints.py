...
from models import Organization, User, Analysis
...
def _setup_two_orgs(db):
    ...
    recruiter = User(supabase_id="rec-a", email="rec@a.test", role="recruiter", organization_id=org_a.id)
    cand_a = User(supabase_id="cand-a", email="cand@a.test", role="individual", organization_id=org_a.id)
    cand_b = User(supabase_id="cand-b", email="cand@b.test", role="individual", organization_id=org_b.id)
    ...
    a_analysis = Analysis(user_id=cand_a.id, organization_id=org_a.id, similarity_score=80.0, interpretation="", confidence=0.0, risk_level="")
    b_analysis = Analysis(user_id=cand_b.id, organization_id=org_b.id, similarity_score=60.0, interpretation="", confidence=0.0, risk_level="")
...
def test_recruiter_sees_own_org_candidates(client, db_session):
    ...
    client.app.dependency_overrides[verify_supabase_jwt] = lambda: {"user_id": recruiter.supabase_id, "email": recruiter.email}
...
def test_list_endpoint_no_leak(client, db_session):
    ...
    for i in range(100):
        c = User(supabase_id=f"cb-{i}", email=f"cb{i}@b.test", role="individual", organization_id=org_b.id)
        db_session.add(c)
        db_session.commit()
        db_session.refresh(c)
        db_session.add(Analysis(user_id=c.id, organization_id=org_b.id, similarity_score=50.0, interpretation="", confidence=0.0, risk_level=""))
        db_session.commit()
...
def test_top_candidates_org_scope(client, db_session):
    ...
    cand_bh = User(supabase_id="cand-b-high", email="high@b.test", role="individual", organization_id=org_b.id)
    db_session.add(cand_bh)
    db_session.commit()
    db_session.refresh(cand_bh)
    db_session.add(Analysis(user_id=cand_bh.id, organization_id=org_b.id, similarity_score=99.0, interpretation="", confidence=0.0, risk_level=""))
    db_session.commit()
...
def test_pagination_and_top_ordering(client, db_session):
    ...
    for t, s in zip(times, scores):
        db_session.add(Analysis(user_id=cand_a.id, organization_id=org_a.id, similarity_score=s, interpretation="", confidence=0.0, risk_level="", created_at=t))
    db_session.commit()
...
def test_top_candidates_filters(client, db_session):
    ...
    db_session.add(Analysis(user_id=cand_a.id, organization_id=org_a.id, similarity_score=50.0, interpretation="", confidence=0.0, risk_level=""))
    db_session.commit()
